...
const Header = ({ mode, currentLevel, levelStyles }: Props) => {
  ...
  return (
    <div className="fixed md:top-4 left-0 right-0 z-50 px-4 py-3 sm:px-6 sm:py-4">
      <div className="max-w-4xl mx-auto relative flex items-center justify-between">
        ...
        {/* タイトル（中央寄せ） */}
        <div className="absolute left-1/2  -translate-x-[65%] text-black is:text-xl sm:text-2xl md:text-4xl font-bold">
          {title}
        </div>

        {/* 🎓 習熟度ラベル（右寄せ） */}
        <div
          className={`absolute right-0 w-[80px] md:w-[160px] text-xs sm:text-lg md:text-2xl font-bold px-1 md:px-5 py-1 md:py-2 rounded-3xl backdrop-blur-sm text-center ${
